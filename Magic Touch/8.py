...
save_last_screen = False 			# A última imagem tirada da tela deve ser salva? Para salvar novos padrões
...
def run_pattern_profile(profile_id):
	...
	pyautogui.moveTo(x = coord[0] + cur_pattern[0][0], y = coord[1] + cur_pattern[0][1])
	...
	for i in range(1, len(cur_pattern)):
		...
		distance = euclidian_distance(last_coord[0], last_coord[1], next_coord[0], next_coord[1])
		swipe_duration = distance / middle_speed

		# Executa movimento
		pyautogui.moveTo(x = coord[0] + cur_pattern[i][0], y = coord[1] + cur_pattern[i][1], duration = swipe_duration)
...
while True:
	...
	if save_last_screen: cv2.imwrite('last_screen.png', game_screen)
	...
	for idx, template in  enumerate(patterns):	
		...
		for current_point in points:
			for target_point in points:
				distance = euclidian_distance(current_point[0], current_point[1], target_point[0], target_point[1])
				if distance <= margin:
		...
		if debug_image:
			for pt in points:
				color = (int(pattern_colors[idx][0]), int(pattern_colors[idx][1]), int(pattern_colors[idx][2]))
				cv2.rectangle(game_screen, pt, (pt[0] + w, pt[1] + h), color, 2)
