...
//circle(0.00f, 0.65f, 0.15f, 16);

int average_read(byte pin, int n)
{
	float acc;
	for(int i = 0; i < n; i++){
		acc += analogRead(pin);
...
void control()
{
	//Rotina de Movimentação por controle Manual
	while(true)
	{
		//Realiza leitura dos potenciometros
		int pot_j1 = average_read(A2, 100);
		int pot_j2 = average_read(A5, 100);
		int pot_j3 = average_read(A1, 100);

		//Configura setpoints para enviar ao V-Rep
		setpoint.t1 = map(pot_j1, 0, 1024, 225, -45);
		setpoint.t2 = map(pot_j2, 0, 1024, -135, 135);
		setpointz   = home_z - (map(pot_j3, 0, 1024, zmin * 1000, zmax * 1000)/1000.0f);

		//Realiza comunicação com o V-Rep
		com_cycle();	//Comunicação X/Y
		com_cycle1();	//Comunicação Z
	}
}
